    ...
    fetch('content.json')
      ...
      .then(data => {
        ...
        for (let i = 0; i < numOfQuestions; i++) {
            ...
            element.classList.add("bg-slate-300", "rounded-md", "border", "px-5", "py-5", "flex", "flex-col", "gap-4", "justify-center", "m-2", "dark:bg-slate-800", "dark:text-white", "dark:border-gray-600", "w-11/12", "max-w-2xl");
...
submitBtn.addEventListener("click", () => {
    ...
    for (let i = 0; i < selectedQuestions.length; i++) {
        ...
        let solutionValue = data.find(x => x.id === selectedQuestions[i]).solution;
